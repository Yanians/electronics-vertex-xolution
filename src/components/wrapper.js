...
export class Emailfield extends React.Component{render(){
  const { validate,required, onChange  } = this.props;
    let classes = {
        validate,
        required,
    }
 try{   
  return (<>
            <TextInput onChange={onChange} error={this.props.error} value={this.props.v} s={this.props.s} m={this.props.m} l={this.props.l} xl={this.props.l} icon={this.props.icon} label={this.props.label}  placeholder={this.props.placeholder||''} id={this.props.id||''} email className={this.props.className} required={cx(classes)} />
         </>
        )   
 }catch{
 } 
